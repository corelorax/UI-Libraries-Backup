...
local drawing  = loadstring(http.request({Url = 'https://gist.githubusercontent.com/0f76/9dc85c8c380d895373dd306fd372fa59/raw/e2abc40c2b5f159d61b10558c86e4f98823e30f5/drawing_extension.lua', Method = 'GET'}).Body)()
local tween = loadstring(http.request({Url = 'https://gist.githubusercontent.com/0f76/1661258383c3c320ac5af2c9dd923fd5/raw/ee3c79b95eafa3b732127a0a7d37a4dc43b3bd60/custom_tween.lua', Method = 'GET'}).Body)()
...
local themeobjects = {};
local library = {priorities = {}, friends = {}, notiflist = {ntifs = {}, interval = 12}, settings = {folder_name = "seereV3/" .. startUpArgs[1];default_accent = Color3.fromRGB(255,255,255)}, drawings = {}, theme = table.clone(themes.Default),currentcolor = nil, flags = {}, open = false, mousestate = services.InputService.MouseIconEnabled, cursor = nil, holder = nil, connections = {}, notifications = {}, gradient = nil};
local decode = (syn and syn.crypt.base64.decode) or (crypt and crypt.base64decode) or base64decode
local flags = {};
local configignores = {};
local visValues = {};
local keys = {
...
function library.createdropdown(holder, content, flag, callback, default, max, scrollable, scrollingmax, section, sectioncontent)
    ...
    local value = library:create("Text", {
        Text = "",
        Font = Drawing.Fonts.Plex, -- default font is Plex
        Size = 13,
...
function library:new(cfg)
    ...
    --
    local window_title = library:create("Text", {Text = name_white, Parent = window_outline, Visible = true, Transparency = 1, Theme = "Accent", Size = 13.5, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0,10,0,7), ZIndex = 13});
    local window_title_accent = library:create("Text", {Text = name_color, Parent = window_outline, Visible = true, Transparency = 1, Theme = "Un-Selected", Size = 13.5, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0,utility.textlength(name_white, 2, 13).X + 10,0,7), ZIndex = 13});
    --
            ...
            function multisection:section(cfg)
                ...
                function section:toggle(cfg)
                    ...
                    function toggle:colorpicker(cfg)
                        ...
                        local colorpicker_tbl = {};
                        colorpicker_tbl.default = default
                        colorpicker_tbl.flag = flag
                        colorpicker_tbl.callback = callback
                        colorpicker_tbl.defaultalpha = defaultalpha
                
                        table.insert(colorpickerz, colorpicker_tbl)
...
function library:createwatermark(info)
    ...
    local sub = info.sub or info.Sub or 'seere'
    title = utility.findtriggers(title) --or title
    ...
    watermark.objects.text2 = library:create("Text", { Parent = watermark.objects.background, Visible = true, Transparency = 1, Theme = "Accent", Size = 13, Text = sub, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0, 3,0,2), ZIndex = 11})
    watermark.objects.text3 = library:create("Text", { Text = title, Parent = watermark.objects.background, Visible = true, Transparency = 1, Theme = "Un-Selected", Size = 13, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0, 1 + utility.textlength(watermark.objects.text2.Text, 2, 13).X,0,2), ZIndex = 11})
    ...
    function watermark.settext(text)
        title = utility.findtriggers(text) -- "{game} | {fps} | " then blah blah blahh
        watermark.update()
    end
    ...
    function watermark.update()
        if watermark.objects.holder.Visible then
            ...
            watermark.objects.text3.Position = UDim2.new(0, 2 + utility.textlength(watermark.objects.text2.Text, 2, 13).X,0,2)
